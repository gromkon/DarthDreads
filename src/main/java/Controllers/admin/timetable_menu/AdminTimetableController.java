package Controllers.admin.timetable_menu;
...
public class AdminTimetableController extends AdminController implements Initializable {
...
//    public Button addNewVisitBtn;
    ...
    public void showInfo(ActionEvent actionEvent) {
        ...
        if (column != -1 & row != 0) {
            if (!timeData.get(row).getText(column).equals("-")) {
                setInfo(column, row);
                SceneController.changeScene((Stage) infoBtn.getScene().getWindow(), "Админ_Расписание_Информация");
            }
        }
    ...
    public void changeVisit(ActionEvent actionEvent) {
        ...
        if (column != -1 & row != 0) {
            if (!timeData.get(row).getText(column).equals("-")) {
                setInfo(column, row);
                SceneController.changeScene((Stage) infoBtn.getScene().getWindow(), "Админ_Расписание_Редактировать");
            }
        }
    }
}
